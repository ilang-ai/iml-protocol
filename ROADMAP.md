...
| 2026-09-18 | 0.5.1 released as v0.5.1, a fix release after an adversarial review of a clean clone of v0.5.0; its testing (43,500 generated documents, 15,000 grammar-built IML texts) broke no law on anything the codec compiled, and IML accepted no document that the validator rejects. Codec: a full-width colon that a same-line trailing token hides and a one-operation chain in preamble position whose print is a tag line are E300 where they stand; a declaration or text line in a 0.4 or 0.3 message is E502; continuation lines are joined in linear time. Command line: an error of the canonical check is reported at the first source line of its item, and every leading byte order mark is dropped. Specification: layout defined, L2 restricted to parsed or decoded ASTs, the one-spelling sentence reworded, the operation lines outside the 0.4 subset listed in §9.1, two messages corrected. The rule-sheet rows of the measurement report are measured from the release tags (the 0.2 figure of 1,607 matched no tag), and the report is regenerated. 19 malformed cases added, 266 in all; 153 tests. Message form, registries, AST and error codes unchanged. |
