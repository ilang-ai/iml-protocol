...
| `RULE-SHEET.md` | The short sheet a model is given to read and write IML 0.4: 1,796 tokens under cl100k_base. |
...
| `corpus/` | 72 golden chains (`golden/*.ilang`) with their expected IML on the 0.3 surface (`golden-0.3/`, which the 0.4 codec reads as is) and on the 0.2 surface (`golden/*.iml`, the record); the 0.4 golden corpus below; 161 malformed inputs with their expected error codes, each naming its surface. |
...
| `tests/` | `python -m unittest discover -s tests`: 87 tests, including 10,000 generated chains under the round-trip law on the 0.4 and the 0.2 surface, the document law on 100 generated documents, a 500-chain sample checked by the canon validator, and the two assertions that tie the 72 golden chains to their 0.3 text. |
...
Python 3.10 or later, standard library only. Input is I-Lang chains on standard input or in a file, one per line, or one per line with continuation lines beginning with `=>`.
...
| 0.4.0 | Released 2026-09-18 as v0.4.0: flow within the canon. The batch shorthand `[BATC:VERB]` and `[Π:VERB]` of canon §3.9 (a verb reference on BATC only, `BT:RD`); multi-line chains joined on compile (canon PATCH-2 §1.7); the three definitions the roadmap demanded, written from canon text; the header `#iml/0.4/`, a 0.3 header read without a flag; `SPEC-IML-0.4.md`; `corpus/golden-0.4/` (36 chains and one document pair); 27 malformed cases added; `measurements/0.4-2026-09-18.md`. Conditionals, parallel groups, DAGs, error handling and retry are not chain syntax in the canon and so not in IML. Registry, value rules, canonical print, round-trip law and error codes unchanged; the AST gains one field. |
